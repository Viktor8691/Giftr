...
Widget GiftItem(Gift item) => Container(
  ...
  child: Column(
    children: [
      [
        [
          ['Name : '.toText(color: AppColors.colorPrimary), item.sender!.name.toText(fontSize: 18),].toRow(),
          5.toVerticalSizedBox,
          ['Phone : '.toText(color: AppColors.colorPrimary), item.sender!.phone_no.toText(fontSize: 18)].toRow(),
        ].toColumn(),
        Spacer(),
        ['Received date'.toText(color: AppColors.colorPrimary, fontSize: 13), getTime(item.createdAt).toText(color: Colors.black54, fontSize: 13)].toColumn()
